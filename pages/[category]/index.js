...
Index.getInitialProps = async ctx => {
  ...
  if (category.includes(".html")) {
    ...
    const resPost = await fetch(`https://45.76.179.13:4043/api/articles/Trangnhat/${articleId}`);
    ...
    const resPost = await fetch(`https://45.76.179.13:4043/api/articles/${typeQuery}?page=${pageNumber || 1}&limit=15`);
    const jsonPost = await resPost.json()
    const data = jsonPost.data;
